...
function renderWires(){const o=selected(),armed=state.armed;
  ...
  setWire('wireAAdder',armed&&state.op==='ADD',bin(state.A));
  setWire('wireBAdder',armed&&state.op==='ADD',bin(state.B));
  setWire('wireAdderMux',armed&&state.op==='ADD',bin(sum()));
  setWire('wireCtrlA',armed&&!!o.loadA);
  setWire('wireCtrlB',armed&&!!o.loadB);
  setWire('wireCtrlMux',armed&&!!o.selA);
}
function render(){
  ...
  const la=state.armed?o.loadA:0,lb=state.armed?o.loadB:0,sa=state.armed?o.selA:0;
  [['sigLoadA',la],['sigLoadB',lb],['sigSelA',sa]].forEach(([id,v])=>{$(id).textContent=v;$(id).classList.toggle('on',!!v)});
  $('ctrlBits').textContent=state.armed?o.bits:'00';$('ctrlState').textContent=state.armed?'ARMED':'IDLE';$('ctrlState').classList.toggle('armed',state.armed);
  $('ctrlMeaning').textContent=state.armed?`${state.op==='LDA'?'LOAD A':state.op==='LDB'?'LOAD B':state.op}: [LOAD_A, LOAD_B, SEL_A] = [${la}, ${lb}, ${sa}]`:'No instruction armed · outputs forced to 0';
  $('microExplain').textContent=state.armed?o.text:'Select an instruction. It will be armed and execute exactly once on the next rising clock edge.';
  ...
  $('muxSel').textContent=sa;$('muxOut').textContent=bin(state.armed&&o.selA?sum():state.bus);$('muxSource').textContent=state.armed&&o.selA?'Adder result selected':'External bus selected';
